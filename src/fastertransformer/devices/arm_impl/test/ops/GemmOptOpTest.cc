...
void ArmGemmOptOpTest::BasicGemmOP(size_t m, size_t n, size_t k) {
    ...
    auto C_host_slice = C_host.index({torch::indexing::Slice(0, 10), torch::indexing::Slice(0, 10)});
    auto C_slice = C.index({torch::indexing::Slice(0, 10), torch::indexing::Slice(0, 10)});

    std::cerr << "C_host_slice: \n" << C_host_slice << std::endl;
    std::cerr << "C_slice: \n" << C_slice << std::endl;
